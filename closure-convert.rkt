  ...
  (define (e->llvm e)
    (match e
           ...
           [`(let ([,x (make-closure ,lamx ,xs ...)]) ,e0)
            ...
            (define arityptr (gensym 'a))
            (define arity (gensym 'a))
            (define aritytmpptr (gensym 'a))
            ...
            (apply string-append
                   `(,(comment-line
                       ...
                       "malloc")
                     ,(comment-line
                         "  %" (s-> arityptr) " = getelementptr inbounds i64, i64* %" (s-> cptr) ", i64 1\n"
                         "  %" (s-> aritytmpptr) " = alloca i64, align 8\n"
                         "  store i64 " (number->string (length xs)) ", i64* %" (s-> aritytmpptr) ", align 8\n"
                         "  %" (s-> arity) " = load i64, i64* %" (s-> aritytmpptr) ", align 8\n"
                         "  store i64 %" (s-> arity) ", i64* %" (s-> arityptr)
           ...
           [`(let ([,x (env-ref ,y ,n)]) ,e0)
            ...
            (apply string-append
                   `(,(comment-line
                     ...
                     ,(comment-line
                       "  %" (s-> iptr) " = getelementptr inbounds i64, i64* %" (s-> yptr) ", i64 " (number->string (+ 1 n))
                       (string-append "&" (s-> yptr) "[" (number->string (+ 1 n)) "]"))
                     ,(comment-line
                       "  %" (s-> x) " = load i64, i64* %" (s-> iptr) ", align 8"
                       (string-append "load; *" (s-> iptr)))
                     ,(e->llvm e0)))]
           [`(let ([,x (prim ,op ,ys ...)]) ,e0)
            (define xptr (gensym 'x))
            (string-append
             (if (memv x '(car cdr cons make-vector hash hash-set)) ; Primitives that allocate space on the heap (put on the stack so GC doesn't clean prematurely)
                 (string-append "\n  %" (s-> xptr) " = alloca i64, align 8\n")
                 "")
             (comment-line
             ...
             (if (memv x '(car cdr cons make-vector hash hash-set))
                 (string-append "\n  store volatile i64 %" (s-> x) ", i64* %" (s-> xptr) ", align 8\n")
                 "")
             (e->llvm e0))]
           [`(let ([,x (apply-prim ,op ,y)]) ,e0)
            (define xptr (gensym 'x))
           ...
           [`(clo-app ,fx ,xs ...)
            ...
            (apply string-append
             `(,(comment-line
                "  call void @assert_closure(i64 %" (s-> fx) ")"
                "check for non-function values")
               ,(comment-line
