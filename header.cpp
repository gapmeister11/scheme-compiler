...
#define CLOSURE_OTHERTAG 0
#define VECTOR_OTHERTAG 1
#define CONS_OTHERTAG 2
#define HASH_OTHERTAG 3
...
#define ASSERT_OTHERTAG(v,tag,msg) \
    if((((v)&7ULL) != OTHER_TAG) || (((((u64 *)(v))[0])&7ULL) != (tag))) \
        fatal_err(msg);
...
#define ENCODE_CLO(v) ENCODE_OTHER(v)
...
u64 encode_cons(u64 *v) {
  u64 *ret = (u64 *)GC_malloc(sizeof(u64) + sizeof(u64 *));
  mem_check();

  ret[0] = CONS_OTHERTAG;
  ret[1] = (u64)v;

  return ((u64)ret | OTHER_TAG);
}
//#define DECODE_CONS(v) ((u64*)((v)&(7ULL^MASK64)))
//#define ENCODE_CONS(v) (((u64)(v)) | CONS_TAG)
#define DECODE_CONS(v) ((u64 *)((u64*)((v)&(7ULL^MASK64)))[1])
#define ENCODE_CONS(v) encode_cons(v)
...
void assert_closure(u64 v) {
  u64 *clo = ((u64 *)v);
  if(((v&7ULL) != OTHER_TAG) || (((clo[0])&7ULL) != CLOSURE_OTHERTAG)) {
    fatal_err("Cannot apply non-function values.\n");
  }
}

void assert_args(u64 fun, u64 cnt) {
  assert_closure(fun);
  ...
  if(clo_cnt < cnt - 1) {
    printf("Given: %08lx\n", cnt);
    printf("Needed: %08lx\n", clo_cnt);
    fatal_err("Too few arguments provided.");
  } else if(clo_cnt > cnt) {
    printf("Given: %08lx\n", cnt);
    printf("Needed: %08lx\n", clo_cnt);
    fatal_err("Too many arguments provided.");
  }
}

constexpr GC_word memcap = 268435500; // 256MiB

void mem_check() { // Check after each heap allocation that we aren't over the limit.
  GC_word heap_size, bytes_free;

  GC_get_heap_usage_safe(&heap_size, &bytes_free, nullptr, nullptr, nullptr);

  if((heap_size - bytes_free) > memcap) {
    fatal_err("Memory cap reached. >= 256 MiB allocated on the heap.");
  }
}
...
{
    ASSERT_OTHERTAG(args, CONS_OTHERTAG, "Expected cons value (in expect_args1). Prim applied on an empty argument list.")
...
u64 expect_cons(u64 p, u64* rest)
{
    ...
    ASSERT_OTHERTAG(p, CONS_OTHERTAG, "Expected a cons value. (expect_cons)")
...
u64 prim_print_aux(u64 v)
{
    if (v == V_NULL)
        ...
        printf("#<procedure:void>");
    else if ((v&7) == OTHER_TAG && (CONS_OTHERTAG == (((u64*)DECODE_OTHER(v))[0] & 7)))
    {
        u64* p = DECODE_CONS(v);
        printf("(");
        prim_print_aux(p[0]);
             ...
             && (VECTOR_OTHERTAG == (((u64*)DECODE_OTHER(v))[0] & 7)))
    {
        ...
        printf(")");
            ...
            && (CLOSURE_OTHERTAG == (((u64*)DECODE_OTHER(v))[0] & 7))) {
      printf("#<procedure>");
    }
    else
...
u64 applyprim_vector(u64 lst)
{
    ...
    u64* buffer = (u64*)GC_malloc(512*sizeof(u64));
    mem_check();
    u64 l = 0;
    while ((lst&7) == OTHER_TAG && (CONS_OTHERTAG == (((u64*)DECODE_OTHER(lst))[0] & 7)) && l < 512)
...
u64 prim_procedure_63(u64 a) // prim procedure?
{
    if ((a&7) == OTHER_TAG && (CLOSURE_OTHERTAG == (((u64*)DECODE_OTHER(a))[0] & 7)))
        return V_TRUE;
    else
        return V_FALSE;
}
...
{
    if ((p&7) == OTHER_TAG && (CONS_OTHERTAG == (((u64*)DECODE_OTHER(p))[0] & 7)))
        return V_TRUE;
    else
        return V_FALSE;
}
...
u64 applyprim__43(u64 p)
{
    ...
    {
        ASSERT_OTHERTAG(p, CONS_OTHERTAG, "Tried to apply + on non list value.")
        u64* pp = DECODE_CONS(p);
...
u64 prim_make_45hash() {
  nedtries::trie_map<u64, u64> *hash = new (GC_malloc(sizeof(nedtries::trie_map<u64, u64>))) nedtries::trie_map<u64, u64>();
  mem_check();
...
u64 prim_hash_45ref(u64 h, u64 k) {
  u64 *hvec = DECODE_OTHER(h);
  nedtries::trie_map<u64, u64> *hash = ((nedtries::trie_map<u64, u64> *)hvec[1]);

  if(hash->find(k) == hash->end()) {
    fatal_err("hash-ref: no value found for key");
  }
